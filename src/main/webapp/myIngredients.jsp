...
<script type="text/javascript">
	...
	app.controller('ingredientsController', function($scope, $http) {
		...
		$scope.openDiscussionDialog = function(ingredient){
			...
			$scope.scenario=3;
			$http.get("IngredientsAndGroupsServlet?ingredientId=" + ingredient.id).
			success(function(data,status,headers,config){
	...
	<input class="search" ng-model="pattern"/>
	...
	<table id="ingredients" border="1">
		<thead><tr>
	  	...
	  	<tbody>
			<tr ng-repeat="ingredient in filteredIngredients = myIngredients | filter: pattern">	  		
				<td>{{ingredient.id}}</td>
